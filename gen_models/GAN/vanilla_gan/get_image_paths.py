...
def tfrecord_auto_traversal():
    current_folder_filename_list = os.listdir("/home/gabbard/Pictures/IGR_staff_image_database") # Change this PATH to traverse other directories if you want.
    if current_folder_filename_list != None:
        print("%s files were found under current folder. " % len(current_folder_filename_list))
        print("Please be noted that only files end with '*.png' will be load!")
